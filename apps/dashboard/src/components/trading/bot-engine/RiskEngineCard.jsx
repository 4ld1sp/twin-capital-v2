      ...
      <div className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-xl border border-[var(--border)] rounded-3xl p-6 flex flex-col h-full relative overflow-hidden">
          ...
          {isLoading && !bot ? (
            ...
            <div className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg border ${
              isActive
                ...
                : 'bg-secondary/10 border-[var(--border)]'
            }`}>
              <span className={`w-1.5 h-1.5 rounded-full ${isActive ? 'bg-emerald-500 animate-pulse' : 'bg-secondary/40'}`} />
              <span className={`text-[9px] font-black uppercase tracking-widest ${isActive ? 'text-emerald-500' : 'text-[var(--text-secondary)]'}`}>
        ...
        {!bot ? (
          ...
          <div className="flex-1 space-y-4 relative z-10">
            ...
            {/* Daily Drawdown Bar */}
            <div className={` border border-[var(--border)] p-4 rounded-xl ${rc.glow}`}>
              <div className="flex justify-between items-center mb-2">
                <div className="flex items-center gap-2">
                  <div className="flex items-center gap-1.5">
                    <TrendingDown className="w-3.5 h-3.5 text-[var(--text-secondary)]" />
                    <span className="text-[9px] text-[var(--text-secondary)] font-black uppercase tracking-widest">Daily Drawdown</span>
                  </div>
                  <button
                    ...
                    className="p-1 px-1.5 rounded-md hover:bg-white/5 text-[8px] text-secondary/60 hover:text-primary border border-transparent hover:border-[var(--border)] transition-all flex items-center gap-1 font-black uppercase tracking-[0.1em]"
                  >
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className=" border border-[var(--border)] p-3 rounded-xl">
                <div className="flex items-center gap-1 mb-1">
                  ...
                  <p className="text-[8px] text-[var(--text-secondary)] font-black uppercase tracking-widest opacity-60">Trail Activation</p>
                </div>
              ...
              <div className=" border border-[var(--border)] p-3 rounded-xl">
                <div className="flex items-center gap-1 mb-1">
                  <Target className="w-3 h-3 text-secondary/60" />
                  <p className="text-[8px] text-[var(--text-secondary)] font-black uppercase tracking-widest opacity-60">Callback Rate</p>
                </div>
                <p className="text-sm font-black text-amber-500 font-mono">
            ...
            <div className="bg-rose-500/5 border border-rose-500/10 rounded-xl p-3.5 flex items-start gap-3">
              ...
              <div>
                <p className="text-xs font-bold text-main/90 leading-relaxed">
                  Monitoring <span className="font-black text-[var(--text-primary)]">3 kill-switches</span> independently of AI.
                  Max positions: <span className="font-black text-[var(--text-primary)]">{bot.maxPositions ?? 1}</span>.
                  Risk per trade: <span className="font-black text-[var(--text-primary)]">{bot.riskPerTradePct ?? 1}%</span>.
                </p>
        ...
        <div className="mt-5 pt-4 border-t border-[var(--border)]/50 flex justify-end relative z-10">
          <button
          ...
          <div className="relative bg-[var(--bg-card,#0f1118)] border border-rose-500/30 rounded-3xl p-8 max-w-md w-full shadow-[0_0_60px_rgba(244,63,94,0.2)]">
            ...
            {bot && (
              <div className="bg-rose-500/5 border border-rose-500/20 rounded-xl px-4 py-3 mb-5 text-center">
                <p className="text-xs text-[var(--text-secondary)] font-bold">Target Bot</p>
            ...
            {/* Description */}
            <p className="text-center text-[var(--text-secondary)] text-sm font-bold leading-relaxed mb-7">
            ...
            <div className="flex gap-3">
              <button
                onClick={() => setIsModalOpen(false)}
                className="flex-1 py-3.5 rounded-xl border border-[var(--border)] text-[var(--text-secondary)] hover:text-white text-xs font-black uppercase tracking-widest transition-colors hover:bg-white/5"
              >
                Cancel
              </button>
              <button
