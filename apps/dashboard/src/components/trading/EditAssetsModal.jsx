    ...
    return (
        <>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-0 pointer-events-none">
                <div className="w-full max-w-md bg-[var(--bg-surface)] border border-[var(--border)] rounded-xl border border-[var(--border)] rounded-3xl shadow-2xl pointer-events-auto transform transition-all">

                    <div className="flex justify-between items-center px-8 py-6 border-b border-[var(--border)]">
                    ...
                    <div className="px-8 py-6">
                        <p className="text-[10px] text-[var(--text-secondary)] font-black uppercase tracking-widest mb-6 opacity-60">
                        ...
                        <form onSubmit={handleAddAsset} className="mb-6 flex gap-3">
                            <input
                                ...
                                placeholder="e.g. XRP"
                                className="flex-1 border border-[var(--border)] rounded-xl px-5 py-3 text-[var(--text-primary)] placeholder:text-secondary/30 placeholder:uppercase placeholder:tracking-widest focus:outline-none focus:border-primary/50 focus:ring-2 focus:ring-primary/20 uppercase text-xs font-black tracking-widest transition-all"
                                maxLength={10}
                            />
                            <button
                                type="submit"
                        ...
                        <div className="space-y-2 mb-2">
                            <div className="flex justify-between text-[10px] font-black text-[var(--text-secondary)] mb-3 uppercase tracking-widest">
                            ...
                            <div className="flex flex-wrap gap-2 max-h-48 overflow-y-auto pr-1 custom-scrollbar">
                                {assets.map((asset) => (
                                    <div key={asset} className="flex items-center gap-2 bg-primary/10 border border-primary/20 text-[var(--text-primary)] px-4 py-2 rounded-xl select-none">
                                        <span className="font-black text-xs uppercase tracking-widest">{asset}</span>
                                        <button
                                            onClick={() => handleRemoveAsset(asset)}
                                            disabled={assets.length <= 2}
                                            className="text-[var(--text-secondary)] hover:text-rose-500 disabled:opacity-30 disabled:hover:text-[var(--text-secondary)] transition-colors flex items-center"
                                        >
                    ...
                    <div className="flex gap-3 px-8 py-6 border-t border-[var(--border)]">
                        <button
                            onClick={onClose}
                            className="flex-1 py-3 rounded-2xl text-[var(--text-secondary)] font-black text-[11px] uppercase tracking-widest hover: transition-all border border-[var(--border)]"
                        >
                            Cancel
                        </button>
                        <button
