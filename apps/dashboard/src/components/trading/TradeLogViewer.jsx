...
const TradeLogViewer = ({ botId, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex flex-col p-4 sm:p-8 bg-black/60 backdrop-blur-sm shadow-2xl overflow-hidden">
      <div className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-xl border border-[var(--border)] rounded-[2rem] w-full max-w-5xl mx-auto bg-black/80 flex flex-col h-full shadow-[0_0_100px_rgba(0,184,217,0.1)] overflow-hidden">
        
        {/* Header */}
        <div className="p-6 border-b border-[var(--border)] flex items-center justify-between shrink-0 bg-black/50">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center border border-[var(--border)]">
              <span className="material-symbols-outlined text-[var(--text-primary)] text-2xl">receipt_long</span>
            </div>
            <div>
              <h2 className="text-xl font-black text-[var(--text-primary)] tracking-tight">AI Trade Logs</h2>
              <p className="text-[var(--text-secondary)] text-xs font-bold font-mono mt-1 flex items-center gap-2">
                ID: {botId}
              </p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 rounded-xl hover:bg-rose-500/10 text-[var(--text-secondary)] hover:text-rose-500 transition-colors">
              ...
              return (
                <div key={log.id} className={`p-4 rounded-2xl border ${cardColor} transition-all hover:scale-[1.01] flex flex-col md:flex-row gap-4 items-start`}>
                  ...
                  <div className="flex sm:flex-col items-center sm:items-start gap-3 w-full sm:w-48 shrink-0">
                    ...
                    <div className="text-[var(--text-secondary)] text-[10px] font-mono font-bold ml-auto sm:ml-0">
                      {new Date(log.createdAt).toLocaleString()}
                    </div>
                    {/* Source Badge */}
                    <div className="hidden sm:inline-flex px-2 py-0.5 mt-2 rounded bg-black/20 border border-[var(--border)] text-[9px] font-black uppercase tracking-widest text-secondary/70">
                  ...
                  <div className="flex-1 space-y-3 min-w-0">
                    ...
                    {(log.qty || log.price || log.pnl) && (
                      <div className="flex flex-wrap gap-2 pt-2">
                        {log.qty && (
                          <div className="px-3 py-1.5 rounded-lg bg-black/20 border border-[var(--border)] flex items-center gap-2">
                            <span className="text-[9px] text-[var(--text-secondary)] font-black uppercase tracking-widest">Qty</span>
                            <span className="text-xs text-[var(--text-primary)] font-mono font-bold">{log.qty}</span>
                          </div>
                        )}
                        {log.price && (
                          <div className="px-3 py-1.5 rounded-lg bg-black/20 border border-[var(--border)] flex items-center gap-2">
                            <span className="text-[9px] text-[var(--text-secondary)] font-black uppercase tracking-widest">Price</span>
                            <span className="text-xs text-[var(--text-primary)] font-mono font-bold">{log.price}</span>
                          </div>
                        )}
                        {log.pnl && (
                          <div className="px-3 py-1.5 rounded-lg bg-black/20 border border-[var(--border)] flex items-center gap-2">
                            <span className="text-[9px] text-[var(--text-secondary)] font-black uppercase tracking-widest">Realized PnL</span>
                        ...
                        {log.trailingStop && (
                          <div className="px-3 py-1.5 rounded-lg bg-black/20 border border-[var(--border)] flex items-center gap-2">
                            <span className="text-[9px] text-[var(--text-secondary)] font-black uppercase tracking-widest">Trailing Trigger</span>
