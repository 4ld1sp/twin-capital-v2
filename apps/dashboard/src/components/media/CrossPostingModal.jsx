    ...
    return (
        <div ref={ref} className="relative">
            <button
                ...
                className="w-full flex items-center justify-between gap-2 border border-[var(--border)] rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary/50 outline-none text-left transition-all hover:  disabled:opacity-50"
            >
                <div className="flex items-center gap-2">
                    <span className={`material-symbols-outlined text-[18px] ${selected.color}`}>{selected.icon}</span>
                    <span className="text-[var(--text-primary)] font-bold">{selected.label}</span>
                </div>
                <span className={`material-symbols-outlined text-[var(--text-secondary)] text-[18px] transition-transform ${open ? 'rotate-180' : ''}`}>expand_more</span>
            </button>

            {open && (
                <div className="absolute top-full left-0 right-0 mt-2 bg-[var(--bg-surface)] backdrop-blur-3xl border border-[var(--border)] rounded-2xl shadow-2xl z-[60] overflow-hidden animate-fade-in-up">
                    {statusOptions.map(option => (
                        <button
                            key={option.value}
                            type="button"
                            onClick={() => { onChange(option.value); setOpen(false); }}
                            className={`w-full flex items-center gap-3 px-4 py-3.5 text-sm transition-all hover:bg-primary hover:text-black ${value === option.value ? 'bg-primary/10 font-black' : 'text-[var(--text-primary)]'}`}
                        >
...
const CrossPostingModal = ({ isOpen, onClose, onSubmit, onUpdate, onDelete, editTask }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            {/* Modal Content */}
            <div className="relative bg-[var(--bg-surface)] border border-[var(--border)] rounded-xl border border-[var(--border)] rounded-3xl shadow-2xl overflow-hidden flex flex-col xl:flex-row w-full max-w-5xl max-h-[90vh] animate-fade-in-up transition-all duration-300">

                {/* Left Column: Composer */}
                <div className="flex-1 p-6 md:p-8 border-b xl:border-b-0 xl:border-r border-[var(--border)] flex flex-col overflow-y-auto">
                    <div className="flex justify-between items-center mb-8">
                        <h3 className="font-black text-2xl flex items-center gap-2 text-[var(--text-primary)]">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <button onClick={resetAndClose} disabled={isPublishing} className="w-10 h-10 rounded-full flex items-center justify-center text-[var(--text-secondary)] hover:  transition-all disabled:opacity-50">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
                        <div className="flex flex-col gap-2">
                            <label className="text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-widest pl-1">Target Post Time</label>
                            <input
                                type="datetime-local"
                                value={targetTime}
                                onChange={(e) => setTargetTime(e.target.value)}
                                disabled={isPublishing}
                                className="border border-[var(--border)] rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary/50 outline-none text-[var(--text-primary)] font-bold transition-all"
                            />
                        </div>
                        <div className="flex flex-col gap-2">
                            <label className="text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-widest pl-1">Pipeline Status</label>
                    ...
                    <div className="flex flex-col flex-1 gap-6">
                        <div className="relative flex-1 min-h-[200px]">
                            <textarea
                                ...
                                className="w-full h-full min-h-[200px] border border-[var(--border)] rounded-2xl p-6 resize-y focus:ring-2 focus:ring-primary/50 text-base text-[var(--text-primary)] placeholder:text-[var(--text-secondary)] disabled:opacity-50 transition-all font-medium"
                            />
                            <div className="absolute bottom-4 right-4 text-[10px] text-[var(--text-secondary)] font-black  px-2 py-1 rounded-lg border border-[var(--border)]">
                        ...
                        <div className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-xl border border-purple-500/30 rounded-2xl overflow-hidden shadow-lg shadow-purple-500/10">
                            <button
                                type="button"
                            ...
                            {showAiPanel && (
                                <div className="p-6 space-y-6 border-t border-purple-500/20 bg-purple-500/[0.02]">
                                    {/* Media Type Tabs */}
                                    <div className="flex gap-2  rounded-xl p-1.5 border border-[var(--border)]">
                                        {[
                                            ...
                                            <button
                                                ...
                                                className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${aiMediaType === tab.id ? 'bg-primary text-black shadow-lg shadow-primary/20' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]'}`}
                                            >
                                    ...
                                    <div className="flex gap-3">
                                        <input
                                            ...
                                            className="flex-1 border border-purple-500/30 rounded-xl px-5 py-3.5 text-sm focus:ring-2 focus:ring-purple-500/50 outline-none text-[var(--text-primary)] placeholder:text-[var(--text-secondary)] disabled:opacity-50 font-medium"
                                        />
                                        <button
                                    ...
                                    {isGenerating && aiMediaType !== 'text' && (
                                        <div className=" border border-purple-500/30 rounded-2xl p-8 flex flex-col items-center gap-4">
                                            ...
                                            <div className="text-center">
                                                ...
                                                <p className="text-[10px] text-[var(--text-secondary)] font-bold mt-1 uppercase tracking-wider">Dreaming up your {aiMediaType}...</p>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {aiMediaType === 'text' && (aiGenerated || isGenerating) && (
                                        <div className="relative">
                                            <div className=" border border-purple-500/30 rounded-2xl p-6 space-y-4">
                                                <div className="flex items-center justify-between">
                                                    <div className="flex items-center gap-2">
                                                ...
                                                <div className="text-sm text-[var(--text-primary)] whitespace-pre-wrap break-words leading-relaxed min-h-[80px] font-medium">
                                                    {aiGenerated}
                                                ...
                                                {!isGenerating && aiGenerated && (
                                                    <div className="flex gap-3 pt-4 border-t border-[var(--border)]">
                                                        ...
                                                        <button onClick={discardAiContent} className="flex items-center justify-center gap-2 px-4 py-3  text-[var(--text-secondary)] rounded-xl text-xs font-black uppercase tracking-widest border border-[var(--border)] hover:bg-rose-500/10 hover:text-rose-500 transition-all">
                                    ...
                                    {aiMediaType !== 'text' && aiGeneratedMedia && !isGenerating && (
                                        <div className=" border border-purple-500/30 rounded-2xl overflow-hidden shadow-xl">
                                            ...
                                            <div className="p-4">
                                                <div className="relative rounded-xl overflow-hidden border border-[var(--border)] shadow-2xl group">
                                                ...
                                                <p className="text-[10px] text-[var(--text-secondary)] font-bold mt-4 italic uppercase tracking-wider">{aiGeneratedMedia.desc}</p>
                                            </div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 items-center">
                            ...
                            <button 
                                ...
                                disabled={isPublishing} 
                                className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3 bg-[var(--bg-surface)] border border-[var(--border)] rounded-xl hover:  rounded-xl text-xs font-black uppercase tracking-widest transition-all border border-[var(--border)] disabled:opacity-50 text-[var(--text-primary)] font-bold"
                            >
                            ...
                            <div className="flex-1 w-full bg-primary/5 border border-primary/30 rounded-xl p-4 flex flex-col sm:flex-row justify-between items-center gap-3">
                                <div className="flex items-center gap-3">
                                    ...
                                    />
                                    <label htmlFor="affiliateToggleModal" className="text-xs font-black uppercase tracking-widest cursor-pointer text-[var(--text-primary)]">Affiliate Tag</label>
                                </div>
                                <input
                                    type="text"
                                    value={affiliateLink}
                ...
                <div className="w-full xl:w-[350px] dark:bg-white/[0.02] p-6 md:p-8 flex flex-col border-t xl:border-t-0 border-[var(--border)] overflow-y-auto">
                    <h4 className="font-black text-[10px] text-[var(--text-secondary)] uppercase tracking-widest mb-6 pl-1">TARGET AUDIENCE NETWORKS</h4>
                    ...
                    <div className="flex-1 flex flex-col gap-4">
                        {platforms.map(platform => (
                            <div
                                ...
                                className={`p-4 rounded-2xl border transition-all cursor-pointer flex items-center justify-between ${platform.active ? 'border-primary/50 bg-primary/5 shadow-lg shadow-primary/5 opacity-100' : 'border-[var(--border)] opacity-40 hover:opacity-70'} ${!platform.connected ? 'cursor-not-allowed grayscale' : 'hover:'} ${isPublishing ? 'pointer-events-none' : ''}`}
                            >
                                <div className="flex items-center gap-4">
                                    ...
                                    <span className={`text-sm font-black uppercase tracking-widest ${platform.active ? 'text-[var(--text-primary)]' : 'text-[var(--text-secondary)] font-bold'}`}>
                                        {platform.name}
                    ...
                    <div className="mt-8 pt-6 border-t border-[var(--border)]">
                        ...
                        <p className="text-center text-[10px] text-[var(--text-secondary)] font-black uppercase tracking-widest mt-5 flex items-center justify-center gap-1.5 opacity-60">
                ...
                {showDeleteConfirm && (
                    <div className="absolute inset-0 z-[110] flex items-center justify-center bg-black/60 backdrop-blur-md">
                        <div className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-xl border border-rose-500/30 rounded-3xl p-10 shadow-3xl max-w-sm w-full mx-4 text-center animate-fade-in-up">
                            ...
                            <h3 className="text-xl font-black mb-3 text-[var(--text-primary)] uppercase tracking-widest">Delete this task?</h3>
                            <p className="text-sm text-[var(--text-secondary)] font-bold mb-8 uppercase tracking-wider leading-relaxed">This action cannot be undone. The task will be vanished from your pipeline.</p>
                            <div className="flex gap-4">
                                <button onClick={() => setShowDeleteConfirm(false)} className="flex-1 py-3.5 rounded-2xl border border-[var(--border)] font-black text-[10px] uppercase tracking-widest hover:  transition-all text-[var(--text-primary)]">Cancel</button>
