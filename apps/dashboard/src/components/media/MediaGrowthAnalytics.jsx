    ...
    <div className="w-full space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {connectedSocials.map(conn => {
          ...
          return (
            <div key={conn.id} className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-xl p-6 rounded-2xl border border-[var(--border)] group hover:border-primary/30 transition-all shadow-sm">
              ...
              <div className="space-y-1">
                <p className="text-[10px] font-black uppercase tracking-widest text-[var(--text-secondary)] opacity-60">{conn.name}</p>
                <div className="flex items-baseline gap-2">
                  <h4 className="text-3xl font-black text-[var(--text-primary)]">{count.toLocaleString()}</h4>
        ...
        {connectedSocials.length === 0 && (
          <div className="col-span-full py-12 flex flex-col items-center justify-center bg-[var(--bg-surface)] border border-[var(--border)] rounded-xl rounded-2xl border-dashed border-2 border-[var(--border)]">
            <span className="material-symbols-outlined text-4xl text-[var(--text-secondary)] mb-4 opacity-20">cloud_off</span>
            <p className="text-[var(--text-secondary)] text-sm font-bold">No API connections active</p>
            <p className="text-[10px] text-[var(--text-secondary)] opacity-60 uppercase tracking-widest mt-1">Connect social accounts in Settings to see growth metrics</p>
          </div>
        )}
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="">
                ...
                <th className="px-6 py-4 text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-widest">Audience</th>
                <th className="px-6 py-4 text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-widest">30D Growth</th>
                <th className="px-6 py-4 text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-widest">Efficiency</th>
            ...
            <tbody className="divide-y divide-glass">
              {connectedSocials.map(conn => {
                const count = conn.stats?.followers || conn.stats?.subscribers || 0;
                return (
                  <tr key={conn.id} className="hover: transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <span className="text-sm font-bold">{conn.name}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm font-mono font-bold text-[var(--text-primary)]">{count.toLocaleString()}</td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                         <div className="flex-1 h-1 rounded-full overflow-hidden max-w-[60px]">
